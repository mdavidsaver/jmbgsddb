...
struct ElementDrift : public Moment2ElementBase
{
    ...
    virtual void recompute_matrix(state_t& ST)
    {
        ...
        transfer(state_t::PS_X, state_t::PS_PX) = L;
        transfer(state_t::PS_Y, state_t::PS_PY) = L;
...
struct ElementQuad : public Moment2ElementBase
{
    ...
    virtual void recompute_matrix(state_t& ST)
    {
        ...
        // Horizontal plane.
        GetQuadMatrix(L,  K, (unsigned)state_t::PS_X, transfer);
        // Vertical plane.
        GetQuadMatrix(L, -K, (unsigned)state_t::PS_Y, transfer);
        // Longitudinal plane.
...
struct ElementSolenoid : public Moment2ElementBase
{
    ...
    virtual void recompute_matrix(state_t& ST)
    {
        ...
        std::cout<<"SOL "<<Brho<<" "<<K<<" "<<L<<" "<<transfer(state_t::PS_S, state_t::PS_PS)
                <<"\n misalign    : "<<misalign
                <<"\n misalign_inv: "<<misalign_inv
                <<"\n transfer    : "<<transfer
                <<"\n";

        scratch  = prod(transfer, misalign);
        transfer = prod(misalign_inv, scratch);
        std::cout<<"SOl  transfer    : "<<transfer<<"\n";
