    ...
    ,phi_ref(std::numeric_limits<double>::quiet_NaN())
{
...

    {
        ...
        std::string rawline;
        unsigned line=0;
        while(std::getline(fstrm, rawline).good()) {
            line++;
            ...
            if(lstrm.eof()) {
                if(needE0) {
                    std::ostringstream strm;
                    strm<<"element "<<params.type<<" requires 5 parameters.  from "<<cavfile<<" near line "<<line;
                    throw std::runtime_error(strm.str());
                }
                params.E0 = 0.0;
            } else {
                if(!needE0) {
                    std::ostringstream strm;
                    strm<<"element "<<params.type<<" requires 4 parameters.  from "<<cavfile<<" near line "<<line;
                    throw std::runtime_error(strm.str());
                }
                lstrm >> params.E0;
            }
